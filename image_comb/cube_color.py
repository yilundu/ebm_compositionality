...
def generate_color_sim():
    idx = random.randint(0, 300)
    color_name = chosen_colors[idx]
...
def generate_whole_sim(rank, near=False):
    ...
    for i in range(chunksize):
        im, label_numpy = generate_color_sim()
...
def generate_sims(traj_num=100000):
    ...
    np.savez("cubes_varied_color_812.npz", ims=ims, labels=labels)
