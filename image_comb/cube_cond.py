...
def get_sim(cube_infos):
    ...
    for i, cube_info in enumerate(cube_infos):
        ...
        xml += """
            ...
            <material name="DEFAULT_GEOM_MAT{0}" texture="DEFAULT_GEOM_TEX{0}" texuniform="false"/>
            """.format(i, *rgba[:-1])
...
def generate_trans_sim():
    color_name = random.choice(chosen_colors[:150])
    color = THECOLORS[color_name]
    rgba = [c / 255. for c in list(color)]
    color = np.array(rgba)
    random_perturb = np.random.uniform(-0.2, 0.2, (4,))
    rgba = np.clip(color + random_perturb, 0, 1)

    bins = np.linspace(-1, 1, 20)
    x, y = random.choice(bins), random.choice(bins)
    size = np.random.uniform(0.3, 0.5)
    types = ["box", "sphere"]

    labels = [{'tx': x, 'ty': y, 'tz': size, 'sx': size, 'sy': size, 'sz': size, 'rgba': rgba, 'type': random.choice(types)}]

    label_numpy = np.zeros(2)
    label_numpy[:] = [labels[0]['tx'], labels[0]['ty']]
...
def generate_whole_sim(rank, near=False):
    ...
    for i in range(chunksize):
        im, label_numpy = generate_trans_sim()
...
def generate_sims(traj_num=100000):
    ...
    np.savez("cubes_varied_position_812.npz", ims=ims, labels=labels)
